...
import {
  CLUB_ITEM_HISTORY_RENTAL_STATUS_MENU,
  type ClubItemHistoryRentalStatus,
} from '@/app/(dashboard)/item-history/_helpers/constants/clubItemHistoryRentalStatus';
...
type Props = {
  filters: {
    ...
    rentalStatusQuery: ClubItemHistoryRentalStatus | DefaultOption;
...
export const ItemHistoryFilter = ({ filters, handlers }: Props) => {
  return (
      ...
      <div className="flex flex-col gap-2 md:flex-row">
        ...
        <div className="flex flex-1 flex-col gap-2">
          ...
          <Select
            ...
            <SelectContent>
              {CLUB_ITEM_HISTORY_RENTAL_STATUS_MENU.map(({ label, value }) => (
                <SelectItem key={value} value={value}>
                  {label}
                </SelectItem>
              ))}
