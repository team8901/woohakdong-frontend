...
export const BillingClient = ({ clubId }: BillingClientProps) => {
  ...
  const handleOpenModal = (plan: SubscriptionPlanId) => {
    ...
    // 등록된 빌링키가 있으면 확인 단계로, 없으면 결제수단 선택 단계로
    setModalStep(
      isFreeDowngrade
        ? 'confirm'
        : defaultBillingKey
      ...
      if (isMockMode) {
        ...
        const paymentResponse = await fetch('/api/portone/billing', {
          ...
          body: JSON.stringify({
            ...
            paymentId,
            orderName: `${plan.name} 플랜 월간 정기결제`,
            amount: billingPrice,
            customer: {
              id: user.uid,
              name: myProfile?.nickname ?? user.displayName,
        ...
        <DialogContent>
          <SelectCardStep
            ...
            onRegisterPaymentMethod={(methodId) => {
              ...
              handleRegisterRealCard(methodId as PaymentMethodId, true);
            }}
            onRegisterMockCard={() => {
              setIsCardRegistrationModalOpen(false);
