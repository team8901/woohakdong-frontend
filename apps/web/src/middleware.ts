...
const DEFAULT_PATH = USER_TYPE_ALLOWED_PATH[DEFAULT_USER_TYPE];

export const middleware = (req: NextRequest) => {
  /** 현재 유저 타입 */
  const userType = (req.cookies.get('userType')?.value ??
    DEFAULT_USER_TYPE) as UserType;

  const url = req.nextUrl.clone();
  const pathname = url.pathname;

  /** 허용 경로 목록 */
  const allowedPaths = USER_TYPE_ALLOWED_PATH[userType] ?? DEFAULT_PATH;
